...
const TARIFAS_URL = 'http://angular-frontend/tarifas';

let actor;

Given('el usuario ha iniciado sesión y está en la página de tarifas', async function () {
  ...
  await actor.driver.get(TARIFAS_URL);
...
Then('debería estar en la página de tarifas', async function () {
  // Verifica URL
  const url = await actor.driver.getCurrentUrl();
  assert.ok(
    url.includes('/tarifas'),
    `Se esperaba estar en /tarifas pero la URL es: ${url}`
  );
});